...
```bash
python3 .agents/skills/bridgearc-trade/scripts/generate_trade_pdf.py
```
...
```bash
ls -la .agents/skills/bridgearc-trade/scripts/generate_trade_pdf.py 2>/dev/null
```

2. If missing, create the scripts directory and a functional PDF generator:
```bash
mkdir -p .agents/skills/bridgearc-trade/scripts
```
...
- **PDF generation fails**: Show the Python error and suggest: "Try running: `python3 .agents/skills/bridgearc-trade/scripts/generate_trade_pdf.py` manually to debug."
