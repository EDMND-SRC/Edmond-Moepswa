...
def generate_report(data, output_path="GEO-REPORT.pdf"):
    ...
    # Title
    report_title = "Unified GEO & SEO Analysis Report" if seo_score else "GEO Analysis Report"
    elements.append(Paragraph(report_title, styles['ReportTitle']))
    elements.append(Spacer(1, 8))

    # Subtitle
    subtext = "Generative Engine & Traditional Search Optimization" if seo_score else "Generative Engine Optimization"
    elements.append(Paragraph(
        f"{subtext} Audit for <b>{brand_name}</b>",
    ...
    if seo_score:
        details_data.append(["SEO Score", f"{seo_score}/100 — {get_score_label(seo_score)}"])
    ...
    # Score gauge
    if seo_score:
        g1 = create_score_gauge(geo_score, 180, 180)
        g2 = create_score_gauge(seo_score, 180, 180)
        
        gauge_table = Table([[g1, g2]], colWidths=[200, 200])
        gauge_table.setStyle(TableStyle([('ALIGN', (0,0), (-1,-1), 'CENTER')]))
        elements.append(gauge_table)
        elements.append(Spacer(1, 10))
        
        c1 = get_score_color(geo_score).hexval()
        l1 = get_score_label(geo_score)
        c2 = get_score_color(seo_score).hexval()
        l2 = get_score_label(seo_score)
        
        label_table = Table([
            [Paragraph(f'<font color="{c1}"><b>GEO:</b> {l1}</font>', ParagraphStyle('C', alignment=TA_CENTER, fontSize=16)),
             Paragraph(f'<font color="{c2}"><b>SEO:</b> {l2}</font>', ParagraphStyle('C', alignment=TA_CENTER, fontSize=16))]
        ], colWidths=[200, 200])
    ...
    # ============================================================
    if seo_score:
        elements.append(Paragraph("Dual Score Breakdown", styles['SectionHeader']))
    else:
    ...
    elements.append(Paragraph(
        "This report was generated by the GEO-SEO Antigravity Analysis Tool. "
    ...
    if len(sys.argv) < 2:
        ...
        sample_data = {
            ...
            "crawler_access": {
                ...
                "ClaudeBot": {"platform": "Antigravity", "status": "Allowed", "recommendation": "Keep allowed"},
