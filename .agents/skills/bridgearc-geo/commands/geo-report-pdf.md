...
- The PDF generation script is located at: `.agents/skills/bridgearc-geo/scripts/generate_pdf_report.py`
...
```json
{
    ...
    "crawler_access": {
        ...
        "ClaudeBot": {"platform": "Antigravity", "status": "Blocked", "recommendation": "Unblock for visibility"}
    }
}
```
...
```bash
python3 .agents/skills/bridgearc-geo/scripts/generate_pdf_report.py /tmp/geo-audit-data.json GEO-REPORT-[brand].pdf
```
...
6. **Run the PDF generator**:
   ```bash
   python3 .agents/skills/bridgearc-geo/scripts/generate_pdf_report.py /tmp/geo-audit-data.json "GEO-REPORT-[brand_name].pdf"
   ```
